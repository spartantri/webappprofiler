...
<xsl:if test="count(@required) > 0">SecRule &amp;ARGS:<xsl:value-of select="./@name"/> "@eq 0" "id:9931733,setvar:tx.score=+<xsl:value-of select="./@ratio" />,pass,msg:'Missing required parameter <xsl:value-of select="./@name"/>'"</xsl:if>
    SecRule ARGS:<xsl:value-of select="./@name" /> &quot;!<xsl:value-of select="./@regexp"/>&quot; id:9931733,t:none,t:urlDecode,pass,setvar:tx.score=+<xsl:value-of select="@score"/>,phase:2<xsl:if test="./@id != ''">,id:<xsl:value-of select="./@id"/></xsl:if>
</xsl:when>
<xsl:otherwise>
<xsl:if test="count(@required) > 0">SecRule &amp;ARGS:<xsl:value-of select="./@name"/> "@eq 0" "id:9931733,setvar:tx.score=+10,pass,msg:'Missing required parameter <xsl:value-of select="./@name"/>'"</xsl:if>
    SecRule ARGS:<xsl:value-of select="./@name" /> &quot;!<xsl:value-of select="./@regexp"/>&quot; id:9931733,t:none,t:urlDecode,pass,setvar:tx.score=+10,phase:2<xsl:if test="./@id != ''">,id:<xsl:value-of select="./@id"/></xsl:if>
</xsl:otherwise>
</xsl:choose>
<xsl:apply-templates />
</xsl:template>
...
<xsl:template match="Method">
    SecRule REQUEST_METHOD &quot;!^<xsl:value-of select="./@value"/>$&quot; id:9931733,phase:2,t:none,log,auditlog,skip:<xsl:value-of select="count(child::*) + 1 + count(child::Parameter)"/>
...
<xsl:template match="Cookie">
    SecRule REQUEST_COOKIES:<xsl:value-of select="./@name"/> &quot;!<xsl:value-of select="./@regexp"/>&quot; &quot;id:9931733,setvar:tx.score=+<xsl:value-of select="@score"/>,phase:2,pass&quot;<xsl:apply-templates />
</xsl:template>
